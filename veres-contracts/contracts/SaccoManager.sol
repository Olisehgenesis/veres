...
contract SaccoManager is Ownable, ReentrancyGuard {
    ...
    mapping(uint256 => mapping(uint256 => mapping(address => bool))) public contributionsMade;
    
    // Approval request mappings
    mapping(uint256 => mapping(address => bool)) public membershipRequests;
    ...
    event MembershipRequested(uint256 indexed saccoId, address indexed requester);
    event MembershipApproved(uint256 indexed saccoId, address indexed member);
    event MembershipRejected(uint256 indexed saccoId, address indexed requester);
    event WithdrawalRequested(uint256 indexed saccoId, address indexed member, uint256 amount);
    event WithdrawalApproved(uint256 indexed saccoId, address indexed member, uint256 amount);
    event WithdrawalRejected(uint256 indexed saccoId, address indexed member, uint256 amount);
    ...
    /**
     * @dev Requests to join a SACCO
     */
    function requestMembership(uint256 saccoId)
        external
        saccoExists(saccoId)
        saccoActive(saccoId)
    {
        ...
        require(!membershipRequests[saccoId][msg.sender], "Request already pending");
        
        // Store request
        membershipRequests[saccoId][msg.sender] = true;
        emit MembershipRequested(saccoId, msg.sender);
    }

    /**
     * @dev Approves a membership request
     */
    function approveMembership(uint256 saccoId, address requester)
        external
        saccoExists(saccoId)
        onlyAdmin(saccoId)
        saccoActive(saccoId)
    {
        require(membershipRequests[saccoId][requester], "No pending request");
        
        // Process joining fee
        require(cUSD.transferFrom(requester, address(this), saccos[saccoId].roundFee), "Fee payment failed");
        
        // Add member
        saccoMembers[saccoId].push(requester);
        saccos[saccoId].memberCount++;
        saccos[saccoId].saccoBalance += saccos[saccoId].roundFee;
        
        // Add SACCO to user's list
        userSaccos[requester].push(saccoId);
        
        // Clear request
        membershipRequests[saccoId][requester] = false;
        
        emit MembershipApproved(saccoId, requester);
        emit MemberAdded(saccoId, requester);
        emit FundsDeposited(saccoId, requester, saccos[saccoId].roundFee);
    }

    /**
     * @dev Rejects a membership request
     */
    function rejectMembership(uint256 saccoId, address requester)
        external
        saccoExists(saccoId)
        onlyAdmin(saccoId)
    {
        ...
        emit MembershipRejected(saccoId, requester);
    }

    /**
     * @dev Direct joining of a SACCO (for backward compatibility or simpler workflow)
     */
    ...
    /**
     * @dev Request a withdrawal from SACCO funds
     */
    function requestWithdrawal(uint256 saccoId, uint256 amount)
        external
        saccoExists(saccoId)
        saccoActive(saccoId)
    {
        ...
        require(withdrawalRequests[saccoId][msg.sender].amount == 0 || 
                withdrawalRequests[saccoId][msg.sender].approved, 
                "Request already pending");
        
        // Create request
        withdrawalRequests[saccoId][msg.sender] = WithdrawalRequest({
            amount: amount,
            timestamp: block.timestamp,
            approved: false
        });
        
        emit WithdrawalRequested(saccoId, msg.sender, amount);
    }

    /**
     * @dev Approve a withdrawal request
     */
    function approveWithdrawal(uint256 saccoId, address member)
        external
        saccoExists(saccoId)
        onlyAdmin(saccoId)
    {
        WithdrawalRequest storage request = withdrawalRequests[saccoId][member];
        ...
        emit WithdrawalApproved(saccoId, member, request.amount);
    }

    /**
     * @dev Reject a withdrawal request
     */
    function rejectWithdrawal(uint256 saccoId, address member)
        external
        saccoExists(saccoId)
        onlyAdmin(saccoId)
    {
        ...
        // Clear request
        delete withdrawalRequests[saccoId][member];
        
        emit WithdrawalRejected(saccoId, member, amount);
    }

    /**
     * @dev Execute an approved withdrawal
     */
    function executeWithdrawal(uint256 saccoId)
        external
        nonReentrant
        saccoExists(saccoId)
    {
        WithdrawalRequest storage request = withdrawalRequests[saccoId][msg.sender];
        require(request.amount > 0, "No pending request");
        require(request.approved, "Request not approved");
        
        Sacco storage sacco = saccos[saccoId];
        require(sacco.saccoBalance >= request.amount, "Insufficient SACCO balance");
        
        uint256 amount = request.amount;
        
        // Update state
        sacco.saccoBalance -= amount;
        delete withdrawalRequests[saccoId][msg.sender];
        ...
        // Transfer funds
        require(cUSD.transfer(msg.sender, amountAfterFee), "Transfer to member failed");
    ...
    /**
     * @dev Gets a list of pending membership requests
     */
    function getPendingMembershipRequests(uint256 saccoId)
        external
        view
        saccoExists(saccoId)
        returns (address[] memory)
    {
        ...
        for (uint256 i = 0; i < saccoMembers[saccoId].length; i++) {
            address potentialRequester = saccoMembers[saccoId][i];
            if (membershipRequests[saccoId][potentialRequester]) {
                count++;
            }
        }
        
        // Create array of pending requesters
        ...
        for (uint256 i = 0; i < saccoMembers[saccoId].length; i++) {
            address potentialRequester = saccoMembers[saccoId][i];
            if (membershipRequests[saccoId][potentialRequester]) {
                requesters[index] = potentialRequester;
                index++;
            }
        }
